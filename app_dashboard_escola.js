...
async function fetchSheet(sheetId, tab){
  ...
  return j.table.rows.map(r => ({ k:r.c[0]?.v, v:r.c[1]?.v }));
}
function rowsToObj(rows){ const o={}; rows.forEach(r=>o[r.k]=r.v); return o; }
...
function syncUI(){
  ...
  $("status").innerText = `Período: ${state.periodoInicio} → ${state.periodoFim} | Marca: ${state.marca} | Unidade: ${state.unidade}`;
}

async function renderFechamento(){
  const map = { Cajazeiras:TABS.CAJ, Camaçari:TABS.CAM, "São Cristóvão":TABS.SAO };
  const prefix = state.marca==="T"?"gt_":state.marca==="P"?"gp_":"ge_";
  const unidades = state.unidade==="TODAS"?Object.keys(map):[state.unidade];
  let html="";
  for(const u of unidades){
    const data = rowsToObj(await fetchSheet(UNIDADES_SHEET_ID, map[u]));
    html += `<div class="card"><h3>${u}</h3>
      Faturamento: ${data[prefix+"faturamento"] ?? "—"}<br>
      Matrículas: ${data[prefix+"matriculas"] ?? "—"}<br>
      Ativos: ${data[prefix+"ativos"] ?? "—"}</div>`;
  }
