...
// ====== FECHAMENTO (substitui a tela antiga de Unidades) ======
function fmtNum(v){
  const n = Number(v);
...
function simNaoBadge(v){
  ...
  if (s === "SIM") return '<span class="badge ok">SIM</span>';
  if (s === "NAO" || s === "NÃO") return '<span class="badge bad">NÃO</span>';
  return '<span class="badge warn">—</span>';
}

function renderFechamento(unidades){
  const elCards = document.getElementById("fech_cards");
  const elGT = document.getElementById("fech_gt");
  const elGP = document.getElementById("fech_gp");
  const elGE = document.getElementById("fech_ge");
  const elPerf = document.getElementById("fech_perf");
  const elTxt = document.getElementById("fech_textos");
  if (!elCards || !elGT || !elGP || !elGE || !elPerf || !elTxt) return;

  // Cards executivos (consolidado GE)
  elCards.innerHTML = unidades.map(u=>{
    const kv = u.kv || {};
    const custoTotal = kv["ge_custo_total_com_invest_dl"] ?? kv["ge_custo_total"];
    return `
      <div class="card">
        <h3 style="margin:0 0 10px">${u.nome}</h3>
        <div class="grid">
          <div class="kpi">Faturamento<strong>${fmtMoney(kv["ge_faturamento"])}</strong></div>
          <div class="kpi">Custo Total<strong>${fmtMoney(custoTotal)}</strong></div>
          <div class="kpi">Geração de caixa<strong>${fmtMoney(kv["ge_geracao_caixa"])}</strong></div>
          <div class="kpi">Ativos<strong>${fmtNum(kv["ge_ativos"])}</strong></div>
          <div class="kpi">Matrículas<strong>${fmtNum(kv["ge_matriculas"])}</strong></div>
        </div>
      </div>
    `;
  }).join("");

  // helper para montar tabela comparativa por prefixo
  const baseRows = [
    { key:"faturamento", label:"Faturamento", fmt:fmtMoney },
    { key:"custo_operacional", label:"Custo Operacional", fmt:fmtMoney },
    { key:"custo_total", label:"Custo Total", fmt:fmtMoney },
    { key:"ticket_medio", label:"Ticket Médio", fmt:fmtMoney },
    { key:"ativos", label:"Número de Ativos", fmt:fmtNum },
    { key:"matriculas", label:"Matrículas", fmt:fmtNum },
    { key:"meta", label:"Meta", fmt:fmtNum },
    { key:"evasao_real", label:"Evasão Real", fmt:fmtNum },
    { key:"inadimplencia", label:"Inadimplência", fmt:fmtPct },
  ];
  ...
  function table(prefix){
    const head = `
      <thead><tr>
        <th>Indicador</th>
        ${unidades.map(u=>`<th class="num">${u.nome}</th>`).join("")}
      </tr></thead>`;
    const body = baseRows.map(r=>{
      return `<tr>
        <td>${r.label}</td>
        ${unidades.map(u=>{
          const kv = u.kv || {};
          return `<td class="num">${r.fmt(kv[`${prefix}_${r.key}`])}</td>`;
        }).join("")}
      </tr>`;
    }).join("");
  ...
  elGT.innerHTML = table("gt");
  elGP.innerHTML = table("gp");

  // GE (consolidado) – linhas específicas
  const geRows = [
    { key:"ge_faturamento", label:"Faturamento", fmt:fmtMoney },
    { key:"ge_custo_operacional", label:"Custo Operacional", fmt:fmtMoney },
    { key:"ge_custo_total_com_invest_dl", label:"Custo Total (c/ invest. e DL)", fmt:fmtMoney },
    { key:"ge_geracao_caixa", label:"Geração de caixa", fmt:fmtMoney },
    { key:"ge_ativos", label:"Número de Ativos", fmt:fmtNum },
    { key:"ge_matriculas", label:"Matrículas", fmt:fmtNum },
    { key:"ge_evasao_real", label:"Evasão Real", fmt:fmtNum },
  ];
  ...
  elGE.innerHTML = (() => {
    ...
    const body = geRows.map(r=>{
      return `<tr>
        <td>${r.label}</td>
        ${unidades.map(u=>{
          const kv = u.kv || {};
          const val = (r.key === "ge_custo_total_com_invest_dl")
            ? (kv["ge_custo_total_com_invest_dl"] ?? kv["ge_custo_total"])
            : kv[r.key];
          return `<td class="num">${r.fmt(val)}</td>`;
        }).join("")}
      </tr>`;
    }).join("");
    return `<div class="tableWrap"><table>${head}<tbody>${body}</tbody></table></div>`;
  ...
  // Performance (SIM/NÃO)
  const perfItens = [
    { key:"perf_resultado_operacional", label:"Resultado Operacional" },
    { key:"perf_aumento_faturamento_total", label:"Aumento do faturamento total" },
    { key:"perf_aumento_margem_ticket", label:"Aumento da margem (ticket médio)" },
    { key:"perf_reducao_custo_total", label:"Redução do custo total" },
    { key:"perf_evasao_controle", label:"Evasão (controle de ativos/alunos)" },
    { key:"perf_fluxo_caixa", label:"Fluxo de caixa (+/-)" },
  ];
  ...
  elPerf.innerHTML = (() => {
    ...
    const body = perfItens.map(it=>{
      return `<tr>
        <td>${it.label}</td>
        ${unidades.map(u=>{
          const kv = u.kv || {};
          return `<td>${simNaoBadge(kv[it.key])}</td>`;
        }).join("")}
      </tr>`;
    }).join("");
    return `<div class="tableWrap"><table>${head}<tbody>${body}</tbody></table></div>`;
  ...
  // Textos
  elTxt.innerHTML = unidades.map(u=>{
    const kv = u.kv || {};
    const obs = kv["obs_observacao"] ?? "—";
    const plano = kv["obs_plano_acao"] ?? "—";
    return `
      <div class="card">
        <h3 style="margin:0 0 10px">${u.nome}</h3>
        <div style="color:#4a5a52"><strong>Observação:</strong></div>
        <div style="margin:6px 0 12px">${obs}</div>
        <div style="color:#4a5a52"><strong>Plano de ação:</strong></div>
...
async function main(){
  ...
  try{
    ...
    // Fechamento – 3 Unidades (p4)
    try{
      const unidades = await carregarUnidades();
      renderFechamento(unidades);
    } catch(e){
      console.error(e);
      // não derruba o dashboard todo se a planilha de unidades falhar
    }
